...
describe(`Test computeVerifier`, function () {
  ...
  [`trinitycore`, `azerothcore`].forEach((serverType) => {
    ...
    it(`Test raise identifier length error for ${serverType}`, () => {
      expect(() =>
        computeVerifier(params[serverType], buf, `a`.repeat(17), `tac`)
      ).to.throw(`The identity should have maximum 16 characters`);
    });

    it(`Test raise identifier length error for ${serverType}`, () => {
      expect(() =>
        computeVerifier(params[serverType], buf, `tic`, `a`.repeat(17))
      ).to.throw(`The password should have maximum 16 characters`);
    });
...
describe(`Test custom params`, function () {
  it(`Test verifier`, () => {
    ...
    const myParam = {
      N_length_bits: 256,
      N: BigInt(
        `0x8A4B645E89E1535BBDAD5B8B290650530801B18EBFBF5E8FAB3C82872A3E9BB7`
      ),
      g: BigInt(`0x7`),
      hash: `sha1`,
    };
    ...
    expect(
      `B1EBEABC2AEE9284E6B48E300BB4D47D05A952A248C2B37DB1DF6143568DB449`
    ).to.equal(strVerifier);
  });
});
